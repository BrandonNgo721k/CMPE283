...
atomic64_t exit_cycles = ATOMIC_INIT(0);
EXPORT_SYMBOL(exit_cycles);
atomic_t exit_array[1028] = ATOMIC_INIT(0);
EXPORT_SYMBOL(exit_array);
atomic_t cycle_counter= ATOMIC_INIT(0);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if (eax == 0x4fffffff)
	{
		printk(KERN_INFO "EAX (0x4FFFFFFF)\n");
		eax = atomic_read(&exit_counter);
		cycle_counter = atomic64_read(&exit_cycles);
		printk(KERN_INFO "Number of exits: %u, Exit Cycles: %llu", eax, cycle_counter);

		// high 32 bits of cycles into ebx
		ebx = (u32)(cycle_counter >> 32);

		// low 32 bits of cycles into ecx
		ecx = (u32)cycle_counter;
		printk(KERN_INFO "ebx: %u, ecx: %u", ebx, ecx);
	}
	else if (eax == 0x4ffffffe)
	{
		...
		if ((ecx >= 0 && ecx < 0xA5) || (ecx > 0x3FF && ecx < 0x404))
		{
			...
			{
				printk(KERN_INFO "Unenabled Exit Code: %u\n",ecx);
				eax = ebx = ecx = edx = 0;
			}
		}
		else
		{
			printk(KERN_INFO "Undefined Exit Code: %u\n",ecx);
			eax = ebx = ecx = 0;
			edx = 0xffffffff;
		}
	}
