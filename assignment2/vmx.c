...
//CMPE283 Add atomic vars
extern atomic64_t time_processing_exits;
...
static void check_ept_pointer_match(struct kvm *kvm)
{
    ...
    kvm_for_each_vcpu(i, vcpu, kvm)
    {
        ...
        {
            to_kvm_vmx(kvm)->ept_pointers_match = EPT_POINTERS_MISMATCH;
            return;
        }
    }
                             ...
                             int pgd_level)
{
    ...
    if (enable_ept)
    {
        ...
        if (kvm_x86_ops.tlb_remote_flush)
        {
            ...
            to_kvm_vmx(kvm)->ept_pointers_match = EPT_POINTERS_CHECK;
            spin_unlock(&to_kvm_vmx(kvm)->ept_pointer_lock);
        }
...
static int handle_ept_violation(struct kvm_vcpu *vcpu)
{
    ...
    error_code |= (exit_qualification & 0x100) != 0 ? PFERR_GUEST_FINAL_MASK : PFERR_GUEST_PAGE_MASK;
 ...
 * assistance.
 */
//Add vmx exit code here for Assignment 2
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
    ...
    u64 start_time;
    u64 time_passed;
    start_time = rdtsc();      //processor timestamp counter
    atomic_inc(&exit_counter); //increment counter every exit
    ...
    //add to end of function on exit Assignment2
    time_passed = rdtsc() - start_time;
    //add time passed to total exit time
    atomic64_add(time_passed, &time_processing_exits);
    //print to kernel
    printk("CPUID (0x4FFFFFFF)\nNumber of Exits: %d, Time spent in exit: %llu", atomic_read(&exit_counter), atomic64_read(&time_processing_exits));
                            ...
                            bool *expired)
{
    ...
    if (vcpu->arch.tsc_scaling_ratio != kvm_default_tsc_scaling_ratio &&
        delta_tsc && u64_shl_div_u64(delta_tsc, kvm_tsc_scaling_ratio_frac_bits, vcpu->arch.tsc_scaling_ratio, &delta_tsc))
        return -ERANGE;
